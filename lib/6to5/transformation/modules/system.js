    ...
    _(statement.declarations)
      ...
      .forEach(function (declaration) {
        ...
        var targetStatement = _.find(exportedStatements, {
          _exportedIdentifierName : assignationRight.id.name
        });
...
SystemFormatter.prototype._addImportModule = function (name, importDescription) {
  this.importedModule[name] = this.importedModule[name] || [];
  importDescription && this.importedModule[name].push(importDescription);
};

SystemFormatter.prototype.import = function (node) {
  this._addImportModule(node.source.value);
};
...
SystemFormatter.prototype.exportSpecifier = function (specifier, node) {
  ...
  if (node.source) {
    if (t.isExportBatchSpecifier(specifier)) {
      ...
      this._addToExportStatements(variableName.name, t.memberExpression(
